...
const cart = {
  lineItems: [
    { name: 'prod1', id: 1, price: price(1100), variant: variant(), quantity: 1 },
    { name: 'prod2', id: 2, price: price(1500), variant: variant(), quantity: 2 }
  ],
  totalPrice: {
    centAmount: 12444
  },
...
describe('[commercetools-helpers] cart helpers', () => {
  ...
  it('returns cart total price', () => {
    expect(getCartTotals(null)).toEqual({ total: 0, subtotal: 0 })
  ...
  it('returns cart product attributes', () => {
    const args = { variant: 'test variant', filters: ['filter'] };
    (getProductAttributes as any) = jest.fn()
      .mockImplementation((variant, filters) => ({ variant, filters }))
